...
type lowHighPercentPair struct {
	in  string
	out *StringLowHighPercentValues
	err error
}

func TestStringLowHighPercent(t *testing.T) {
	v := StringLowHighPercentValues{}
	var _ pflag.Value = &v
	wanted := []lowHighPercentPair{
		{
			in:  "",
		...
		{
			in:  "2",
			out: &StringLowHighPercentValues{Low: 2, High: 0},
		},
	}
	for _, want := range wanted {
