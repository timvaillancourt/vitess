	...
	tests := []struct {
	...
	}{
		{
			...
			result: mysqlDirsResult(map[string]string{
				...
				"innodb_parallel_doublewrite_path": "/parallel-doublewrite/xb_doublewrite",
			...
			want: []string{"/data/mysql", "/data/mysql/tmp-one", "/tmp/two;keep", "/data/mysql/innodb", "/data/mysql/innodb/system", "/data/mysql/system-two", "/data/mysql/innodb/temp-system", "/data/mysql/undo", "/innodb-temp", "/data/mysql/#doublewrite", "/parallel-doublewrite", "/binlog", "/binlog-index", "/relay", "/relay-index", "/data/mysql"},
		},
		{
			name:   "relative InnoDB file paths default to datadir",
			result: mysqlDirsResult(map[string]string{"datadir": "/mysql/data", "innodb_data_file_path": "../system/ibdata1:12M:autoextend", "innodb_temp_data_file_path": "temp-volume/ibtmp1:12M:autoextend", "innodb_parallel_doublewrite_path": "doublewrite/xb_doublewrite"}),
		...
		{
			name:   "disabled doublewrite directory",
			result: mysqlDirsResult(map[string]string{"datadir": "/data/mysql", "innodb_doublewrite": "OFF", "innodb_doublewrite_dir": "/unused-doublewrite"}),
