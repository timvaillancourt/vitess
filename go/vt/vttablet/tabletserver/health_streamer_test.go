...
func TestHealthStreamerBroadcast(t *testing.T) {
	...
	hs.ChangeState(topodatapb.TabletType_REPLICA, time.Time{}, 0, nil, false, 1.234)
	shr = <-ch
	want = &querypb.StreamHealthResponse{
		Target: &querypb.Target{
	...
	now := time.Now()
	hs.ChangeState(topodatapb.TabletType_PRIMARY, now, 0, nil, true, 1.234)
	shr = <-ch
	want = &querypb.StreamHealthResponse{
		Target: &querypb.Target{
	...
	hs.ChangeState(topodatapb.TabletType_REPLICA, now, 1*time.Second, nil, false, 1.234)
	shr = <-ch
	want = &querypb.StreamHealthResponse{
		Target: &querypb.Target{
			TabletType: topodatapb.TabletType_REPLICA,
	...
	// Test Health error.
	hs.ChangeState(topodatapb.TabletType_REPLICA, now, 0, errors.New("repl err"), false, 1.234)
	shr = <-ch
	want = &querypb.StreamHealthResponse{
		Target: &querypb.Target{
			TabletType: topodatapb.TabletType_REPLICA,
