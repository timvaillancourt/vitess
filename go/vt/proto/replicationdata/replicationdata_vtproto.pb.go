...
func (m *FullStatus) CloneVT() *FullStatus {
	...
	r.PrimaryVttabletUnreachable = m.PrimaryVttabletUnreachable
	if len(m.unknownFields) > 0 {
		r.unknownFields = make([]byte, len(m.unknownFields))
		copy(r.unknownFields, m.unknownFields)
	}
...
func (m *FullStatus) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.PrimaryVttabletUnreachable {
		i--
		if m.PrimaryVttabletUnreachable {
			dAtA[i] = 1
		} else {
...
func (m *FullStatus) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 23:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field PrimaryVttabletUnreachable", wireType)
			}
			var v int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.PrimaryVttabletUnreachable = bool(v != 0)
		default:
			iNdEx = preIndex
			skippy, err := protohelpers.Skip(dAtA[iNdEx:])
			if err != nil {
