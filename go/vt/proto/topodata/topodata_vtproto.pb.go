...
func (m *Shard_TabletControl) CloneVT() *Shard_TabletControl {
	...
	if rhs := m.DeniedTables; rhs != nil {
		tmpContainer := make([]string, len(rhs))
		copy(tmpContainer, rhs)
		r.DeniedTables = tmpContainer
	}
	if len(m.unknownFields) > 0 {
		r.unknownFields = make([]byte, len(m.unknownFields))
		copy(r.unknownFields, m.unknownFields)
...
func (m *ThrottlerConfig_MetricNames) CloneVT() *ThrottlerConfig_MetricNames {
	if m == nil {
...
func (m *VtorcConfig) CloneVT() *VtorcConfig {
	...
	if rhs := m.RangeTags; rhs != nil {
		tmpContainer := make(map[string]*KeyRange, len(rhs))
		for k, v := range rhs {
			tmpContainer[k] = v.CloneVT()
		}
		r.RangeTags = tmpContainer
	}
	if len(m.unknownFields) > 0 {
		r.unknownFields = make([]byte, len(m.unknownFields))
		copy(r.unknownFields, m.unknownFields)
...
func (m *VtorcConfig) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.RangeTags) > 0 {
		for k := range m.RangeTags {
			v := m.RangeTags[k]
			baseI := i
			size, err := v.MarshalToSizedBufferVT(dAtA[:i])
			if err != nil {
...
func (m *VtorcConfig) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field RangeTags", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.RangeTags == nil {
				m.RangeTags = make(map[string]*KeyRange)
			}
			var mapkey string
			var mapvalue *KeyRange
			for iNdEx < postIndex {
				entryPreIndex := iNdEx
				var wire uint64
				for shift := uint(0); ; shift += 7 {
