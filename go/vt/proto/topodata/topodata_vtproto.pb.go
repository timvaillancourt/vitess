...
func (m *Tablet) CloneVT() *Tablet {
	...
	r.RecordVersion = m.RecordVersion
...
func (m *Tablet) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.RecordVersion != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.RecordVersion))
		i--
		dAtA[i] = 0x1
		i--
		dAtA[i] = 0x88
...
func (m *Tablet) SizeVT() (n int) {
	...
	if m.RecordVersion != 0 {
		n += 2 + protohelpers.SizeOfVarint(uint64(m.RecordVersion))
	}
	n += len(m.unknownFields)
	return n
}
	...
	for iNdEx < l {
		...
		case 17:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.RecordVersion |= uint64(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
