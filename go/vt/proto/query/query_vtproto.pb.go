...
func (m *ExecuteOptions) CloneVT() *ExecuteOptions {
	...
	r.MaxExecutionTimeHint = m.MaxExecutionTimeHint
...
func (m *ExecuteOptions) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.MaxExecutionTimeHint != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.MaxExecutionTimeHint))
		i--
		dAtA[i] = 0x1
		i--
		dAtA[i] = 0xa8
...
func (m *ExecuteOptions) SizeVT() (n int) {
	...
	if m.MaxExecutionTimeHint != 0 {
		n += 2 + protohelpers.SizeOfVarint(uint64(m.MaxExecutionTimeHint))
	}
	n += len(m.unknownFields)
	return n
}
...
func (m *ExecuteOptions) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 21:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field MaxExecutionTimeHint", wireType)
			}
			m.MaxExecutionTimeHint = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return protohelpers.ErrIntOverflow
				}
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.MaxExecutionTimeHint |= int64(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
