...
// GetProblemAnalysis will check for problems (dead primary; unreachable primary; etc)
func GetProblemAnalysis(keyspace string, shard string, hints *ProblemAnalysisHints) ([]*ProblemAnalysis, error) {
	var result []*ProblemAnalysis
	appendAnalysis := func(analysis *ProblemAnalysis) {
	...
	// TODO(sougou); deprecate ReduceProblemAnalysisCount
	...
	err := db.Db.QueryVTOrc(query, args, func(m sqlutils.RowMap) error {
		...
		case topo.IsReplicaType(a.TabletType) && ca.primaryAlias == "" && a.ShardPrimaryTermTimestamp == "":
	...
	// TODO: result, err = getConcensusProblemAnalysis(result)
	return result, err
}
...
func postProcessAnalyses(result []*ProblemAnalysis, clusters map[string]*clusterAnalysis) []*ProblemAnalysis {
	for {
